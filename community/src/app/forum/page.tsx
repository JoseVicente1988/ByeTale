...
function attachmentFallback(attachment: ForumAttachment | null) {
  if (!attachment) return "";
  if (attachment.kind === "video") return "Vídeo adjunto.";
  if (attachment.kind === "audio") return "Audio adjunto.";
  ...
  async function handleReply(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const result = await neon.from("posts").insert({
        ...
        author_id: profile.id,
        body: body || attachmentFallback(attachment),
        ...attachmentColumns(attachment),
      });
                      ...
                      <article className="post" key={post.id}>
                        ...
                        <div className="postBody">
                          ...
                          {post.media_url && post.media_kind === "video" && (
                            <video className="postMedia postVideo" controls preload="metadata" src={post.media_url}>
                              Tu navegador no puede reproducir este vídeo.
                            </video>
                          )}
                          {post.media_url && post.media_kind === "audio" && (
                            <audio className="postMedia postAudio" controls preload="metadata" src={post.media_url}>
                    ...
                    <div className="attachmentRow">
                      ...
                      {replyAttachment && (
                        <span className="attachmentName">
                          <strong>{replyAttachment.name}</strong> · {(replyAttachment.size / 1024 / 1024).toFixed(1)} MB · {forumAttachmentLimitLabel(replyAttachment)}
                        </span>
                      )}
                    </div>
                    <small className="attachmentHelp">Imagen ≤ 3 MB · vídeo MP4/WEBM ≤ 25 MB · audio MP3/OGG/WAV/M4A ≤ 12 MB · ZIP próximamente</small>
            ...
            <div className="modalBody">
              <form className="modalForm" onSubmit={handleCreateThread}>
                ...
                <label className="fileField">
                  ...
                  <small>Imagen PNG/JPG/WEBP ≤ 3 MB · vídeo MP4/WEBM ≤ 25 MB · audio MP3/OGG/WAV/M4A ≤ 12 MB</small>
                  <small className="attachmentHelp">ZIP: arquitectura preparada, activación posterior con validación específica.</small>
                </label>
