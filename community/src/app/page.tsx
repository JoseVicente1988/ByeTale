...
const participation = [
  {
    ...
    icon: "✦",
    title: "Ideas",
    copy: "Propón sistemas, contenido y mejoras. Las propuestas pueden debatirse, recibir apoyo y obtener una respuesta oficial."
  },
  {
    index: "02",
    icon: "◉",
...
export default function HomePage() {
  return (
    <main>
        ...
        <div className="shell heroLayout">
          <div className="heroCopy">
            ...
            <div className="actions">
              ...
              <a className="button secondary" href="#roadmap">Explorar roadmap</a>
            </div>
      ...
      <section className="section participationSection" id="participa"><div className="shell"><div className="sectionHead"><div><span>Entra en el proceso</span><h2>La comunidad ya tiene un lugar donde trabajar.</h2></div><p>Ideas, casting, bugs y colaboración viven en el foro de ByeTale. Los votos muestran interés; la dirección final del juego sigue perteneciendo a ByeTale.</p></div><div className="participation">{participation.map((item) => <article className="participationCard" key={item.title}><span className="participationIndex">{item.index}</span><i aria-hidden="true">{item.icon}</i><h3>{item.title}</h3><p>{item.copy}</p><a href="/forum">Abrir en el foro <span aria-hidden="true">→</span></a></article>)}</div></div></section>

      <section className="section finalSection"><div className="shell"><article className="manifesto"><div><span className="label gold">Principio del proyecto</span><h2>La comunidad participa. ByeTale mantiene su visión.</h2><p>Transparencia sobre qué se está construyendo, por qué se toman decisiones y qué aportaciones de la comunidad terminan formando parte del juego.</p></div><a className="button primary" href="/forum">Entrar al foro</a></article></div></section>

      <footer><div className="shell footer"><div className="brand"><Image className="brandIcon footerIcon" src="/byetale-icon.svg" alt="" width={38} height={38} unoptimized /><span className="brandCopy"><strong>ByeTale</strong><small>Community &amp; Development</small></span></div><p>Godot · ENet · foro · roadmap · testing · voces</p><a href="https://github.com/JoseVicente1988/ByeTale" target="_blank" rel="noreferrer">GitHub <span aria-hidden="true">↗</span></a></div></footer>
    </main>
  );
}
